...
# This code implements step 2 of the analysis (see pdf). Briefly, this means that it cleans
# data by eliminating LOB states outside normal trading hours (9:30 - 16:00).
...
for file in Path(folder).glob('*.csv'):
    ...
    df_new = df.drop(columns = ['Domain', 'Date-Time', 'GMT Offset', 'Type', 'Time_Adj', 'Check', 'Time'])

    df_new.to_csv(file, index = False)

    #df_new.to_csv(file+r'.txt', sep='\t', index = False)